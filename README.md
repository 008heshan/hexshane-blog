...
面板 = **顶部白色高光 + 极低 alpha 中性白膜 + 实时背景模糊**，全部无色相：

| 令牌（`sci-fi.css` 顶部） | 默认值 | 作用 |
| --- | --- | --- |
| `--sf-glass` | `rgba(255,255,255,.055)` | 白膜浓度（调小更透，调大更"厚"） |
| `--sf-glass-2` | `rgba(255,255,255,.075)` | 浮层（检索弹窗）用 |
| `--sf-glass-sheen` | 顶部白 7% → 1.2% | 玻璃反光面（元素自身高光，不是给背景染色） |
| `--sf-glass-line` / `-strong` | 白 12% / 20% | 描边与悬停描边 |
| `--sf-glass-blur` | `20px` | 实时模糊半径 |

- 用在：首页卡片、侧栏卡片、文章面板、文章版权块、检索弹窗、代码块、引用块、行内代码、
  标签胶囊、分页按钮；`theme-glass.css`（dark 卡片 / `#page` / 移动端）与之共用同一批令牌
- **为什么高光和白膜必须存在**：Chromium 在硬件加速不可用或 GPU 进程回退时会**静默丢掉
  `backdrop-filter`**，只靠实时模糊的"玻璃"会直接消失（看起来像没做效果）。所以元素自身
  的高光 + 白膜负责质感，blur 只负责锦上添花；另附 `@supports not (backdrop-filter…)` 分支，
  没有 blur 时自动把白膜加厚一档
- 刻意**不开** `backdrop-filter` 的地方：`#page` / `#post` / `#archive` 长文容器（背后就是
