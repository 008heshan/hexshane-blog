...
写图格（Tile）、弹幕（Projectile）或者绘制相关的代码时，很多人会被 Terraria 里五花八门的坐标系绕晕。这篇把它们一个个讲清楚。
...
玩家、NPC、弹幕（Projectile）、尘埃（Dust）用的都是世界坐标（World Coordinates）。它通常写成 `Vector2`，也就是说 `X` 和 `Y` 都是 `float`，可以带小数 —— `3943.23`、`2334.213` 这种非整数完全合法。屏幕缩放为 100% 时，显示器上的 1 像素恰好等于世界空间的 1 个单位；世界坐标 `0, 0` 就落在整个世界的左上角。

## 图格坐标

图格用的是图格坐标（Tile Coordinates），它只出现在与图格相关的方法以及 `Main.tile` 数组里。图格坐标一般用 `Point` 或 `Point16` 表示，因此 `X`、`Y` 必须是整数。
...
图格并不一定总是画在屏幕上的同一处。如果你在写 `ModTile.SpecialDraw`、`PostDraw` 或 `PreDraw`，代码里要先写上 `Vector2 zero = Main.drawToScreen ? Vector2.Zero : new Vector2(Main.offScreenRange);`，然后给每一处 `spriteBatch.Draw` 调用都加上这个 `zero`。这么做是为了兼容切换光照模式（Lighting mode）时绘制坐标发生的偏移。按 `Shift+F9` 可以快速切换光照模式，方便验证你的代码在各种模式下都画得对。

## 坐标转换

### 图格 <--> 世界坐标

每个图格占据世界坐标空间里 16x16 的一块区域。把图格坐标乘以 16，得到的世界坐标就指向该图格的左上角。用 `Vector2.ToWorldCoordinates` 能把这段计算简化掉；`Point.ToWorldCoordinates` 还会自动给 X 和 Y 各加 8，直接返回图格中心的世界坐标。
...
手动绘制图格时，要记得前面提到的 `Main.drawToScreen`。
...
这个例子的换算过程是这样的：白色线条画出图格的格子范围，红点标出各个图格换算成世界坐标后落在哪里。右下角那个图格的图格坐标是 `3548, 256`，乘以 16 就得到世界坐标 `56768, 4096`。蜗牛的位置显示在黄框里 —— 实体的坐标锚在它自己的左上角，所以把刚才那个图格的世界坐标再加上 `0, 16`，得到 `56768, 4112`，和上面那只蜗牛的头顶位置非常接近（那只蜗牛实际还比这个点往右偏了 1 像素）。
