...
本篇讲的是所有 tile entity（物块实体）共通的那套东西。前提是你已经会建文件、写类，也知道"继承"是什么意思。ExampleMod 里的 [BasicTileEntity.cs](https://github.com/tModLoader/tModLoader/blob/stable/ExampleMod/Content/TileEntities/BasicTileEntity.cs) 就是一个最基本的物块实体，可以对着它一起看。

# 什么是 Tile Entity？

首先得分清 `Tile`（物块）和 tile entity：物块位于世界里的固定坐标，只能通过 `Main.tile` 访问，它存不了额外数据，也没办法在游戏更新时跑代码；tile entity 则是挂在某个 `Tile` 上的一坨附加数据，自带一个 `Update` 方法，通常和物块配合起来让物块有动态行为。

两者的区别就在更新频率上：tile entity 每个游戏 tick 都能跑代码，而普通物块只有等世界更新循环对它调用 `RandomUpdate` 时才有机会跑一次。

所以简单说，只要一个物块需要存额外数据、或者需要稳定地跑代码，就得给它绑一个对应的 tile entity。
...
严格来说这几个方法不是必需的，但物块实体十有八九得存自己的数据、还得把它们同步出去 —— 这本来就是它存在的主要理由。存读数据看[存档与读档](https://github.com/tModLoader/tModLoader/wiki/Saving-and-loading-using-TagCompound)，网络同步看[网络同步指南](https://github.com/tModLoader/tModLoader/wiki/Basic-Netcode#modtileentity)，不熟的话先补一下。一般来说，值得存盘的东西也同样值得同步：同步没做对，客户端跟实体交互时读到的就是错的值。
...
前提是先做好一个普通的 `frameImportant` 多物块（multitile，写法见 [Basic ModTile Guide](https://github.com/tModLoader/tModLoader/wiki/Basic-Tile)）。在此基础上只要再加几行，就能让它被放置时自动生成 tile entity。
...
拿到实例之后能做的事很多：开一个 UI、刷物品、显示提示文字、显示 tooltip、自定义物块的视觉效果等等。[BasicTileEntityTile](https://github.com/tModLoader/tModLoader/blob/stable/ExampleMod/Content/TileEntities/BasicTileEntity.cs#L96) 里那几个方法就是 `TileEntity.TryGet` 的正确用法示例。
