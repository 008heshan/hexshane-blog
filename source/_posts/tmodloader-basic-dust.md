...
Dust 就是那些给武器和各种效果添上视觉细节的小粒子。有一点得先说死：**Dust 纯粹是视觉表现，永远不该拿它当游戏机制**。因为 Terraria 生成多少 dust 取决于玩家电脑的性能，配置吃紧时数量就会被削减 —— 把伤害判定、状态切换之类的东西挂在这些粒子上，换台机器结果就不一样了。

每颗 dust 都配了一张贴图，还各自带着一套行为：有的受重力影响，有的不受；有的像被风卷起的碎屑那样乱飘，有的一直走直线。这些行为都写死在 dust 自己身上，不是调用的时候现配的。
...
`NewDust` 用得最多；`NewDustPerfect` 跳过了生成位置的随机化（就在你给的坐标上冒出来），某些场合反而更顺手。另一个容易踩的差别在返回值：`NewDust` 返回的是这颗 dust 在 `Main.dust` 数组里的下标，另外两个直接把 `Dust` 实例交给你。
...
几个要点：`Position`、`Width`、`Height` 三个参数划出一个矩形，dust 会在这个矩形范围内随机取点冒出来。生成原版 dust 和生成自己模组的 dust，差别只在第 4 个参数 —— 平时填数字或者 `DustID` 里的成员，换成 `ModContent.DustType<DustName>()` 就是你自己写的那颗。剩下那些可选参数不需要就省略。

# 特殊图案

想让 dust 摆出图案，靠的是 [Geometry](https://github.com/tModLoader/tModLoader/wiki/Geometry) 那篇里的几何知识：可以一口气生成一批 dust，也可以分好几次游戏更新陆续生成，用一堆 dust 拼出你要的形状。

# 查找原版 Dust

原版 dust 是排在一张大对照表里的，读法是：从左往右、编号从 0 开始，每行排满 100 颗；纵向上每个位置其实是同一颗 dust 的 3 帧动画叠在一起，整张表一共 3 行。

完整清单也可以查 [Terraria Wiki 的 Dust IDs 页面](https://terraria.wiki.gg/wiki/Dust_IDs)。有些原版 dust 官方压根没给名字，tModLoader 自己补了一批；如果官方 wiki 上找不到某个内部名，就去 [DustID.TML.cs](https://github.com/tModLoader/tModLoader/blob/stable/patches/tModLoader/Terraria/ID/DustID.TML.cs) 里翻，缺的名字都写在那儿。

不过真要挑 dust，最省事的做法还是装一个 [Modders Toolkit](https://forums.terraria.org/index.php?threads/modders-toolkit-a-mod-for-modders-doing-modding.55738/)，用它的 dust 生成工具边看效果边找，需要哪颗当场就能定位。
...
`ModDust` 是自定义 dust 的基类，自己写的 dust 都要继承它。原版 dust 的贴图一律是 10x30 像素、纵向叠着同一颗粒子的 3 帧动画；`ModDust` 没这个限制，尺寸多大、几帧都随你。ExampleMod 里有很多不同写法的现成例子，照着改最快。
